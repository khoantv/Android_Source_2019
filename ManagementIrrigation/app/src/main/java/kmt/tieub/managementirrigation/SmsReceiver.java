...
import kmt.tieub.dbAccess.InsertHistory;
                ...
                for(int k=0; k < msgs.length; k++) {
                    ...
                    if (phone.equals(phoneSent))
                    {
                        if (body.contains("D"))  // Nếu là tin nhắn cập nhật giá trị NĐ, ĐÂ, ...
                        {
                            ...
                            System.out.println("json History: "+ jSon);

                            //region -- Send SMS --
                            //Main2Activity main2Activity = new Main2Activity();
                            MainActivity.getMainActivity().controlPump(history.getDeviceId(), timeIrrgation);

                            InsertHistory insertHistory = new InsertHistory(mContext);
                        ...
                        else if (body.contains("T"))   // Nếu là tin nhắn cập nhật trạng thái thiết bị
                        {
                            ...
                            System.out.println("json Update Tool Status: "+ convertToBool(tb1) +";"+  convertToBool(tb2) +";"+  convertToBool(tb3));
                            // Xử lý cập nhật trạng thái lên server.
                            SetDeviceStatus updateStatus = new SetDeviceStatus(mContext);
                            updateStatus.execute(convertToBool(tb1)+"", convertToBool(tb2)+"", convertToBool(tb3)+"");
                        }
                    }
                }
            }
